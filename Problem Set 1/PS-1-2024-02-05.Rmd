...
Using the summary command, I obtain the following table:

```{r}
PS_1_data <- read_dta("/Users/lionelchambon/Desktop/LIONEL/LIONEL_edu/MASTER/M1/Cours/Econometrics_II/Data/ee2002ext_reload.dta")
PS_1_data <- PS_1_data %>% na.omit()
summary(PS_1_data)
```

### Question 2

I proceed similarly for the variable *salfr*:
...
I use the kable.Extra package i Found here: <https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html>
...
Building a labelled table following the code of *ddipl1*:
...
Question: Why does category 3 show up twice?

###Question 5

I proceed similarly to add mean wages:
...
Question: Why do I have to use *arrange* here? I was stuck on this for a while until I found it online.
...
0 refers to people with no education at all, and 99 to people who are still in school.
...
Perhaps some form of trimming could be used to eliminate some of the outliers? The data also looks very dense, so there is no clear tendency/relationship between our variables of interest visible? However, that could also be an issue with the design of the plot...
...
The 0,5th percentile means that 99,5% of respondents/observations have an income superior to 6.684612, while the 99,5th percentile indicates that 0,5% of observations have an income greater or equal to 10.596635.

### Question 13

Doesn't R automatically store values in the environment?
...
How can I put two plots side-by-side?

The filter appears to suppress outliers/extreme values.

### Question 18

a)  The regression table tells us that an additional unit of "Graduate_Age", that is, an additional year of schooling, increases log income by 0.0487 units. (How do we convert?)
...
When using the command cor, the estimate is incredibly small, proving the desired result. However, when using cor.test, my p-value returns as 1, which can't be right?
...
The symmetric, bell-shaped distribution of residuals is indicative of a normal distribution of the variance
...
```{r}
library(readxl)
PS_1_data_2 <- read_excel("/Users/lionelchambon/Desktop/LIONEL/LIONEL_edu/MASTER/M1/Cours/Econometrics_II/Data/problem_2_ps_1.xlsx")
head(PS_1_data_2)
```
...
The intercept tells us the value of the variable of interest with no treatment - think of consumption if income is zero. The interpretation here is that the the student with the lowest ACT score in the sample had a GPA of 2.8, but we cannot make general conclusions due to the small sample size.
...
To find the variation, we need to compute the R\^2:
